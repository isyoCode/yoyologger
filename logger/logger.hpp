...
namespace yoyo {
    ...
    */
    ...
    public:
        CirculQueen() = default;
        /*留个空位作为 空/满的标识位置 */
        explicit CirculQueen(size_t maxSize) : _iMaxItem(maxSize + 1), _vQueen(_iMaxItem){} 
        CirculQueen(const CirculQueen&) = default;
        CirculQueen& operator=(const CirculQueen&) = default;
        CirculQueen(CirculQueen&& other){
        ...
        ~CirculQueen() = default;
        size_t getNum() const {
            return (_Tail + _iMaxItem - _Head) % _iMaxItem;
        }
    ...
    public:
        ...
        explicit BufferQueen(size_t buffer_size): _dataQueen(CirculQueen<T>(buffer_size)){}
        ...
        void dequeen(std::vector<T>& vecBuffer, size_t _batchSize){
            {
                std::unique_lock<std::mutex> lock(_Mtx);
                _isPopCv.wait_for(lock, std::chrono::milliseconds(1), [this](){ return !_dataQueen.isEmpty();});
    ...
    public:
        ...
        explicit LocationInfo(const std::source_location& loc = std::source_location::current()):
            _Line(loc.line()),
    ...
    explicit Message(LOGLEVEL level, std::string str, LocationInfo tLoc)
        ...
        void outPutMsg(std::ofstream& ofs, bool isOutFile=true, bool _isColor=true){
            // 日志格式: [time][level][filename][function][line][msg]
            std::string logMsg = "[" + getCurrentTime() + "]" 
                + "[" + std::string(LevelFlag[static_cast<int>(_levle)]) + "]"
                + "[" +_loction._fileName + "]"
                + "[" +_loction._Function + "]"
                + "[" + std::to_string(_loction._Line) + "]"
                + "[" + _sMsg + "]" + "\n";
            if(isOutFile){
    ...
    public:
        std::string getCurrentTime() noexcept {
            ...
            char buffer[32];
            snprintf(buffer, sizeof(buffer), "%4d-%02d-%02d %02d:%02d:%02d.%03d", curtime.tm_year + 1900, curtime.tm_mon + 1,
    ...
    private:
        ...
        void writeMsgbuffer(){
            for(auto& msg: _writeBuffer){
                ...
                if( _logcof._isWritefile && (_fileStringBuffer.size() > _fileCurrentBufferSize)){
                    if(_logout.is_open()){
                ...
                if(_logcof._isConsle && _consoleStringBuffer.size() > _consoleCurrentBufferSize){
    ...
    private:
        ...
        std::string getLogDirName() const {
            std::string dirpath;
            return dirpath.append(_logcof._logPrefixPath).append("/").append(_logcof._logDirName);
        }
    ...
    private:
        ...
        void initiallize(){
            ...
            size_t    _batchSize = _iQueenBufferSize >> 1;        // batch size circulQueen size / 2
